//! Analytic phase signals and numerical acceptance thresholds.
...
pub mod groundtruth {
    ...
    pub fn residual(recovered: &[Complex64], truth: &[Complex64]) -> f64 {
...
pub fn run() -> std::process::ExitCode {
    ...
    for (label, n, peaks, p0, p1, noise, method, limit) in cases {
        match recover_n(n, &peaks, p0, p1, noise, method) {
            Ok((residual, estimated_p1)) => {
                println!(
                    "{label}: residual={residual:.12}, required <{limit}; estimated endpoint p1={estimated_p1:.8}deg"
                );
