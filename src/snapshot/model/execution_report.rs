...
const _: () = {
    ...
    impl Codec for ExternalAlgorithmDeclaration {
        fn to_wire<'a>(&'a self, budget: &mut Budget<'_, '_>) -> Result<Value<'a>, SnapshotError> {
            record(
                "execution_report.ExternalAlgorithmDeclaration.v1",
                vec![
        ...
        fn from_wire(
            ...
            let mut f = fields(node, "execution_report.ExternalAlgorithmDeclaration.v1", 5)?;
